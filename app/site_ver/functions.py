...
mins = {}
for level in levels:
    min = open("app/site_ver/data/{}.txt".format(level), encoding='utf-8').read().lower().split('\n') # получаем список слов
    mins[level] = set(min)
#print(mins)
...
def reading(file):
    f = open('app/uploads/files/text.txt'.format(file), 'r', encoding='utf-8')
    text = f.read()
    while '  ' in text:
        text = text.replace('  ', ' ')
    text = text.replace('-\n', '')
...
def writing(task, task_name):
    f = open('app/uploads/to_download/task_{}.txt'.format(task_name), 'w')
    f.write(task)
    f.close()
...
def writing_key(key, task_name):
    f = open('app/uploads/to_download/answer_{}.txt'.format(task_name), 'w')
    f.write(key)
    f.close()
